...
CREATE TABLE `events_all` ON CLUSTER `prod` AS `events_local`
ENGINE = Distributed(`prod`, `db`, `events_local`, rand());
...
CREATE TABLE `backup-table` AS `source-table`
ENGINE = MergeTree()
ORDER BY `id`;
...
CREATE TABLE `base_copy` AS `base`
ENGINE = MergeTree()
ORDER BY `id`;
