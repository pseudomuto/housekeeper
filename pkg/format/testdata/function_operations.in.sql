...
CREATE FUNCTION normalizedOS ON CLUSTER warehouse AS (os) -> multiIf(startsWith(lower(os), 'windows'), 'Windows', startsWith(lower(os), 'mac'), 'Mac', lower(os) IN ('ios', 'iphone'), 'iOS', lower(os) = 'android', 'Android', 'Other');
...
CREATE FUNCTION categorizeValue AS (val) -> multiIf(val < 0, 'negative', val = 0, 'zero', val < 100, 'small', val < 1000, 'medium', 'large');
